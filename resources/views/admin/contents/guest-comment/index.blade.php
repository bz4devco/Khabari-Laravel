...
<title>نظرات کاربران مهمان | پنل مدیریت</title>
@endsection
...
<nav aria-label="breadcrumb">
    <ol class="breadcrumb m-0 font-size-12">
        ...
        <li class="breadcrumb-item active" aria-current="page">نظرات کاربران مهمان</li>
    </ol>
</nav>
<!-- category page Breadcrumb area -->
    ...
    <section class="col-12">
        <section class="main-body-container">
            ...
            <section class="d-flex justify-content-between align-items-center mt-4 pb-3 mb-3 border-bottom">
                <a href="{{ route('admin.content.comment.index') }}" class="btn btn-sm btn-info text-white">لیست نظرات کاربران سایت</a>
                    ...
                    <tbody>
                        ...
                        <tr class="align-middle {{$bg_notif}}">
                            ...
                            <td>
                                <section>
                                    <div class="custom-switch custom-switch-label-onoff d-flex align-content-center" dir="ltr">
                                        <input data-url="{{ route('admin.content.guest-comment.status', $comment->id) }}" onchange="changeStatus(this.id)" class="custom-switch-input" id="{{ $comment->id }}" name="status" type="checkbox" @if($comment->status) checked @endif>
                            ...
                            <td class=" text-start">
                                ...
                                <a href="{{ route('admin.content.guest-comment.show', $comment->id ) }}" class="btn btn-info btn-sm text-white"><i class="fa fa-eye ms-1 align-middle"></i>نمایش</a>
                                @endcan
                                @can('approved-content-comment')
                                <button {{$attr_checked}} type="button" data-url="{{ route('admin.content.guest-comment.approved', $comment->id) }}" onclick="approvedstatus(this.id)" id="{{ $comment->id }}-approved" class="btn btn-{{$btn_style}} btn-sm"><i class="fa fa-{{$icon_style}} ms-1 align-middle"></i>{{$btn_text}}</button>
                                @endcan
                            </td>
                        </tr>
                        @empty
